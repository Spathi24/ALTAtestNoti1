...
| `JOBCOST` | **Actuals ledger** (canonical = gold-standard template, see below) | `Cost_Ledger`, `Scope_Budget`, `Change_Orders`, `Order_Quantities` | See `Parser_Contract` sheet inside each file | XlsxParser → Cost_Ledger shape |
...
### JOBCOST: gold-standard template is canonical, the mock is a demo only

`docs/JOB_COST_TEMPLATE_structured.xlsx` (owner-built) is the **canonical** JOBCOST
workbook — `Project_Setup`, `Cost_Ledger`, `Scope_Budget`, `Change_Orders`,
`Order_Quantities`, `Cost_Code_Summary`, `Progress_Valuation`, `Dashboard`, `Lists`,
`Parser_Contract`. It has not yet been deployed to any real project's Drive folder,
but it is the intended production artifact.

`docs/templates/mock_drive/2026001 — Rockland/JOBCOST/2026001_JOBCOST.xlsx` (generated
by `create_template_drive.py`) is a **pilot-scale mock** that mirrors the gold standard's
sheet roles at smaller scale, for Drive-organization and parser-shape demonstration only.
It is NOT a competing or simplified replacement model.

**Production rule:** for each real project, copy `JOB_COST_TEMPLATE_structured.xlsx`
into the project folder as `{project_code}_JOBCOST.xlsx` and use **its own**
`Parser_Contract` as the ingestion contract. The surrounding SOW / PKG / QUOTE / PO /
BUDGET / GREENSHEET files are supporting project-control documents around it, not
alternative financial sources of truth.

### Data provenance (so nothing here is mistaken for real data)

- **Gold JOBCOST shape** = `docs/JOB_COST_TEMPLATE_structured.xlsx` — the structure/template contract.
- **CSI division/trade list** = real Rockland project DB query (`project_db.sqlite`) — see
  the trade table above; this part reflects actual project shape.
- **All dollar amounts, vendor names, line descriptions, statuses** in the mock Drive =
  **mock examples only**, generated by `create_template_drive.py`. None of it is migrated
  or authoritative financial data. Do not copy these numbers into real reports.
...
```
Description | Masterformat | Material Amount | Labour Amount | Total Amount | Item_ID | Coverage_Y_N | Mat_Incl | Exclusions | Notes | SOW_Item_Ref
```
...
- `SOW_Item_Ref` (added Phase 3, 2026-07-02) is the **structural join key** back to the SOW
  item this quote line prices (e.g. `SOW-025`), not the quote-local `Item_ID` (`QI-001`).
  It must be its own column — never parse a join key out of `Notes` text (e.g. "per SOW-025").
  Phase 4 reads this column directly to set `FinancialLineItem.sow_item_id`.

**Row structure:**

| Row | Description col | Masterformat col | Material Amount | Labour Amount | Total Amount | SOW_Item_Ref |
|---|---|---|---|---|---|---|
| Section-total | Trade name (e.g. "Plumbing") | CSI code (e.g. "22") | empty | empty | trade total | empty |
| Line items | Item description | empty or CSI code | material $ | labour $ | empty | `SOW-025` etc. |
| Pre-Tax Total | **empty** | empty | empty | empty | grand total | empty |
