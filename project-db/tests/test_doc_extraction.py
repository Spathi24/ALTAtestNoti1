...
class TestStructuredExtraction:
    def test_transactional_quote_stored_primary(self, session, project_factory):
        ...
        session.commit()
        ext = MockStructuredExtractor(by_name={"Quote.pdf": {
            "document_type": "construction_quote", "is_transactional": True,
            "summary": "quote", "records": [{
                "amount": 5000, "currency": "CAD", "direction": "client_in",
                "record_kind": "total", "description": "reno",
                "quoted_excerpt": "Total renovation cost: $5,000.00", "confidence": 0.9}]}})
    ...
    def test_market_report_skipped(self, session, project_factory):
        ...
        session.commit()
        ext = MockStructuredExtractor(by_name={"Market Report.pdf": {
            "document_type": "market_report_or_valuation", "is_transactional": False,
            "summary": "market report", "records": [{
                "amount": 2500000000, "currency": "CAD", "direction": "unknown",
                "record_kind": "total", "description": "projected",
                "quoted_excerpt": "Total Investment $2.5 billion", "confidence": 0.5}]}})
        batch = extract_financials_structured_for_project(session, ext, p.canonical_id)
    ...
    def test_budget_kept_as_rollup(self, session, project_factory):
        ...
        session.commit()
        ext = MockStructuredExtractor(by_name={"C61 budget.xlsx": {
            "document_type": "budget_or_cost_tracker", "is_transactional": False,
            "summary": "budget", "records": [{
                "amount": 400000, "currency": None, "direction": "contractor_out",
                "record_kind": "total", "description": "budget total",
                "quoted_excerpt": "Total 400000", "confidence": 0.7}]}})
        batch = extract_financials_structured_for_project(session, ext, p.canonical_id)
        assert batch.created_count == 1
    ...
    def test_all_or_nothing_on_failure_keeps_prior(self, session, project_factory):
        ...
        session.add(FinancialRecord(project_id=p.canonical_id, document_id=d.canonical_id,
                                    direction="client_in", record_kind="total", amount=1))
        session.commit()
    ...
    def test_zero_and_unparseable_amounts_skipped(self, session, project_factory):
        ...
        session.commit()
        ext = MockStructuredExtractor(by_name={"Q.pdf": {
            "document_type": "construction_quote", "is_transactional": True,
            "summary": "q", "records": [
                {"amount": 0, "currency": None, "direction": "client_in",
                 "record_kind": "total", "description": "", "quoted_excerpt": "", "confidence": 0.5},
            ]}})
        batch = extract_financials_structured_for_project(session, ext, p.canonical_id)
