...
class TestFinancialScore:
    ...
    def test_model_regex_matches_analysis_sheets(self):
        for n in [
            "Multifamily Acquisition Model v3.xlsx",
            "Erik_Proforma.xlsx",
            "Scoring Model.xlsx",
...
class TestCandidateSelection:
    ...
    def test_923_rockland_docs_now_selected(self, session, project_factory):
        ...
        self._doc(
            session,
            p,
            "Multifamily Acquisition Model.xlsx",
            XLSX_MIME,
            "### Model\nIRR\t0.18\nNOI\t500000",
        )  # should be skipped
        session.commit()
...
class TestRollupRecompute:
    ...
    def _seed(self, session, p, name, *, amount, direction, body=""):
        d = Document(name=name, url=f"x://{name}", mime_type=XLSX_MIME, project_id=p.canonical_id)
        session.add(d)
        session.flush()
        if body:
            session.add(
    ...
    def test_projection_content_doc_excluded_despite_innocent_name(self, session, project_factory):
