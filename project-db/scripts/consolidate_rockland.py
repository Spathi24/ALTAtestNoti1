...
def main(apply: bool) -> int:
    ...
    from project_db.db.models import (
        ContractObligation, Document, DocumentChunk, ExternalId,
        FinancialRecord, Project, SourceSystem, Task,
    )
    ...
    with session_scope() as s:
        ...
        if survivor is None:
            ...
            if survivor and not losers:
                print("Already consolidated -- one Rockland project named "
        ...
        moved = {"docs": 0, "frs": 0, "obs": 0, "tasks": 0, "chunks": 0,
                 "dailylogs": 0}
        for loser in losers:
            ...
            drive_exts = [e.external_key for e in s.query(ExternalId)
                          .filter_by(canonical_id=lid).all()]
            print(f"\nLoser: {loser.name!r} [{str(lid)[:8]}]")
            print(f"    move docs={d} FRs={fr} obligations={ob} tasks={tk}")
            print(f"    externalids: {drive_exts}")

            if apply:
                for cls, key in ((Document, "docs"), (FinancialRecord, "frs"),
                                 (ContractObligation, "obs"), (Task, "tasks"),
                ...
                for e in s.query(ExternalId).filter_by(canonical_id=lid).all():
                    if e.source == SourceSystem.GOOGLE_DRIVE and \
                       e.entity_type == "Project" and \
                       e.external_key.startswith(TEAM_FOLDER_KEY_PREFIX):
        ...
        if apply:
            ...
            for e in s.query(ExternalId).filter_by(
                    canonical_id=sid, source=SourceSystem.GOOGLE_DRIVE,
