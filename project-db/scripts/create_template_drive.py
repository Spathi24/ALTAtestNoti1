...
SOW_ITEMS = [
    ...
    (
        ...
        "Overhead, profit (15%), and contingency (3%)",
        "Y",
        "",
        "",
    ),
    ...
    (
        ...
        "02",
        "Demolition",
        "Demo existing bathroom fixtures and tile",
        "Y",
        "",
        "All debris removed",
    ),
    ...
    (
        ...
        "03",
        "Concrete",
        "Concrete patching at subfloor penetrations",
        "Y",
        "",
        "If required",
    ),
    (
        ...
        "05",
        "Structural",
        "Structural engineer review for load-bearing elements",
        "Y",
        "",
        "",
    ),
    (
        ...
        "05",
        "Structural",
        "Steel beam / lintel supply and install if required",
        "Y",
        "",
        "Conditional",
    ),
    (
        ...
        "06",
        "Carpentry",
        "Install new door frames and trim throughout",
        "Y",
        "Finger-jointed pine",
        "",
    ),
    ...
    (
        ...
        "07",
        "Roofing",
        "Roof replacement with ventilation system",
        "Y",
        "Fibreglass shingles, 25-yr",
        "",
    ),
    ...
    (
        ...
        "08",
        "DoorsWindows",
        "Supply and install interior doors",
        "Y",
    ...
    (
        ...
        "08",
        "DoorsWindows",
        "Supply and install sliding closet doors",
        "Y",
        "Mirrored bypass",
        "",
    ),
    ...
    (
        ...
        "09",
        "Finishes",
        "Tile floor in bathroom incl. waterproofing membrane",
        "Y",
        "12x24 porcelain, client-selected",
        "",
    ),
    (
        "SOW-020",
    ...
    (
        ...
        "09",
        "Finishes",
        "LVP flooring throughout living areas",
        "Y",
        "Lifeproof 6mm, client-selected",
        "",
    ),
    (
        "SOW-022",
    ...
    (
        ...
        "10-12",
        "Fixtures",
        "Supply and install bathroom vanity",
        "Y",
        '36" single sink, white',
        "",
    ),
    ("SOW-024", "10-12", "Fixtures", "Supply and install countertop", "Y", "Laminate standard", ""),
    ("SOW-025", "22", "Plumbing", "Rough-in plumbing (drain, supply, vent)", "Y", "PEX-A", ""),
    (
        "SOW-026",
        "22",
        "Plumbing",
        "Supply and install plumbing fixtures (toilet, sink, shower)",
        "Y",
        "Moen Adler series",
        "",
    ),
    ...
    (
        ...
        "23",
        "HVAC",
        "Supply and install bathroom exhaust fan",
        "Y",
        "90 CFM, ENERGY STAR",
        "",
    ),
    (
        ...
        "23",
        "HVAC",
        "Electric baseboard heaters throughout",
        "Y",
        "Stelpro 1500W per room",
        "",
    ),
    (
        "SOW-030",
        "26",
        "Electrical",
        "Electrical rough-in (circuits, panel upgrade)",
        "Y",
        "200A panel",
        "",
    ),
    (
        ...
        "26",
        "Electrical",
        "Supply and install light fixtures throughout",
        "Y",
        "LED, client-selected",
        "",
    ),
...
def build_package(path: Path, csi: str, trade: str) -> None:
    ...
    items = [[i[0], i[0], i[3], i[5], "", "", i[6]] for i in SOW_ITEMS if i[1] == csi]
    if not items:
        items = [
...
def build_quote(
    ...
    lines: list[
        tuple
    ],  # (desc, csi_hint, mat, lab, tot, coverage, mat_incl, excl, notes, sow_item_ref)
) -> None:
    ...
    item_rows = []
    for idx, (desc, csi_hint, mat, lab, tot, cov, mat_incl, excl, notes, sow_item_ref) in enumerate(
        lines, 1
    ):
        item_rows.append(
            [
...
def build_greensheet(path: Path) -> None:
    ...
    rows = [
        ...
        ["10-12", "Fixtures", 9000, 8800, 9200, "", 8800, 8800, 8800, 0, 8800, "pending"],
...
def build_budget(path: Path) -> None:
    ...
    rows = [
        ...
        ["BUD-009", "10-12", "Fixtures", 8800.00, 1.00, 8800.00],
    ...
    for r, (k, v) in enumerate(
        [
            (
                "Client_Price_Pre_Global",
                "Equals Budget_Amount x Line_Markup_Factor (per-line inflation only).",
            ),
            (
                "Final Client Price",
                "Equals SUM(Client_Price_Pre_Global) x 1.15 (global 15% markup applied at subtotal).",
            ),
            (
                "Rule",
                "This snapshot is frozen. Do not edit after owner approval. Create v2 for revisions.",
            ),
        ],
        3,
    ):
...
def build_jobcost(path: Path) -> None:
    ...
    for r, (k, v) in enumerate(
        [
            (
                "NOT the canonical template",
                "This file is a lightweight pilot-scale mock for Drive-organization "
            ...
            (
                "Production rule",
                "For each real project, copy JOB_COST_TEMPLATE_structured.xlsx into the project "
            ...
            (
                "Purpose of this mock",
                "Demonstrate the same sheet roles at smaller scale: actuals (Cost_Ledger) + "
                "forecast (Scope_Budget) + change orders + order quantities.",
            ),
            (
                "Main input tables",
                "tblCostLedger, tblScopeBudget, tblChangeOrders, tblOrderQuantities.",
            ),
            (
                "Append rule",
                "Append rows inside named tables. Do not build summaries from fixed row ranges.",
            ),
    ...
    _sheet(
        ...
        [
            ...
            "Item",
            "Unit_Price",
            "Quantity_Per_Affected_Unit",
            "Affected_Units",
            "Tax_Included",
            "Tax_Rate",
            "Amount_Incl_Tax",
            "Amount_Excl_Tax",
            "Include_Current_Forecast",
            "Include_Quote_Total",
            "Include_Target_Budget",
            "Notes",
        ],
        [
            [
                ...
                "22",
                "Unit 923",
                "Plumbing (Plombert Inc. selected quote)",
                6800.00,
                1,
                1,
                "Yes",
            ...
            [
                ...
                "09",
                "Unit 923",
                "Tile allowance (12x24 porcelain)",
                3500.00,
                1,
                1,
                "Yes",
                ...
                "No",
                "Yes",
                "Client-selected, pending invoice",
            ],
            [
                "BUD-0003",
                "Quote_Total",
                ...
                "09",
                "Unit 923",
                "Finishes (ABC Tile pending quote)",
                21500.00,
                1,
                1,
                "Yes",
    ...
    _sheet(
        ...
        [
            [
                "CO-001",
                "Extra demo",
                "Additional demo for hidden rot in subfloor",
                800.00,
                0.15,
                920.00,
                "Pending",
    ...
    _sheet(
        ...
        [
            [
                ...
                "Material",
                "09",
                "Porcelain tile 12x24 (box/10 sqft)",
                10,
                28,
                280,
                "Open",
...
def main() -> None:
    ...
    for csi, trade in TRADES:
        build_package(
            PROJECT_DIR / "SOW" / "packages" / f"{PC}_PKG_{csi.replace('-', '')}-{trade}.xlsx",
            csi,
            trade,
        )
    ...
    build_quote(
        ...
        [
            (
                "Bathroom tile supply and install",
                "09",
                4200.00,
                3800.00,
                8000.00,
                ...
                "Y",
                "",
                "12x24 porcelain; incl. waterproofing",
                "SOW-040",
            ),
            (
                "Drywall and taping throughout",
                "09",
                1800.00,
                4200.00,
                6000.00,
            ...
            (
                "LVP flooring supply and install",
                "09",
                2800.00,
                1700.00,
                4500.00,
